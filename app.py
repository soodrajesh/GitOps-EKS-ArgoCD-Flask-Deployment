...
# Set at import time, not inside `if __name__ == "__main__"`, so it's also
# populated when the app is served by gunicorn (which imports `app` directly
# and never runs that block) rather than only under the Flask dev server.
